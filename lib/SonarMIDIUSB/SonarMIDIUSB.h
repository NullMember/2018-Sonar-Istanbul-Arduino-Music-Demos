...
class SonarMIDIUSB{
    ...
    public:
        ...
        void sendRawBytes(byte status, byte d1, byte d2);
};
...
inline void SonarMIDIUSB::sendRawBytes(byte status, byte d1, byte d2){
  midiEvent = {uint8_t(status & 0x0F), uint8_t(status), uint8_t(d1), uint8_t(d2)};
  MidiUSB.sendMIDI(midiEvent);
}
